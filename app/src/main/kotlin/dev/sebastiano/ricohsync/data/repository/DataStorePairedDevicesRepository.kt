 ...
 */
class DataStorePairedDevicesRepository(
    private val dataStore: DataStore<PairedDevicesProto>,
) : PairedDevicesRepository {

    override val pairedDevices: Flow<List<PairedDevice>> =
        dataStore.data.map { proto ->
            proto.devicesList.map { it.toDomain() }
        }

    override val enabledDevices: Flow<List<PairedDevice>> =
        pairedDevices.map { devices ->
            devices.filter { it.isEnabled }
        }

    override suspend fun addDevice(camera: Camera, enabled: Boolean) {
        dataStore.updateData { currentData ->
            // Check if device already exists
            val existingIndex = currentData.devicesList.indexOfFirst {
                it.macAddress == camera.macAddress
            }

            val newDevice = PairedDeviceProto.newBuilder()
            ...
            if (existingIndex >= 0) {
                ...
                currentData.toBuilder()
                    .setDevices(existingIndex, newDevice)
                    .build()
            } else {
                // Add new device
                currentData.toBuilder()
    ...
    override suspend fun removeDevice(macAddress: String) {
        dataStore.updateData { currentData ->
            val filteredDevices = currentData.devicesList.filter {
    ...
    override suspend fun setDeviceEnabled(macAddress: String, enabled: Boolean) {
        dataStore.updateData { currentData ->
            val deviceIndex = currentData.devicesList.indexOfFirst {
                it.macAddress == macAddress
            }

            if (deviceIndex < 0) return@updateData currentData

            val updatedDevice = currentData.devicesList[deviceIndex].toBuilder()
                .setEnabled(enabled)
                .build()

            currentData.toBuilder()
                .setDevices(deviceIndex, updatedDevice)
                .build()
        }
    }
    ...
    override suspend fun updateDeviceName(macAddress: String, name: String?) {
        dataStore.updateData { currentData ->
            ...
            val updatedDevice = currentData.devicesList[deviceIndex].toBuilder()
                .apply {
                    if (name != null) setName(name) else clearName()
                }
                .build()
    ...
    override suspend fun isDevicePaired(macAddress: String): Boolean {
        return dataStore.data.first().devicesList.any {
            it.macAddress == macAddress
        }
    }

    override suspend fun getDevice(macAddress: String): PairedDevice? {
        return dataStore.data.first().devicesList
            .find { it.macAddress == macAddress }
            ?.toDomain()
    }
 ...
 */
private fun PairedDeviceProto.toDomain(): PairedDevice = PairedDevice(
 ...
 */
val Context.pairedDevicesDataStoreV2: DataStore<PairedDevicesProto> by dataStore(
    fileName = "paired-devices-v2.pb",
    serializer = PairedDevicesProtoSerializer,
)
